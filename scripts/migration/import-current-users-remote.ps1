...
# TriggerFeed V3 Remote Auth/Profile Migration
#
# Imports current legacy TriggerFeed users/profiles into REMOTE V3 Supabase.
#
# Requirements:
...
#
# Usage:
#   $env:OLD_TRIGGERFEED_DB_URL="postgresql://..."
#   $env:TRIGGERFEED_V3_DB_URL="postgresql://..."
#   .\scripts\migration\import-current-users-remote.ps1
...
if (-not $TargetDbUrl) {
  throw "TRIGGERFEED_V3_DB_URL is not set. Set it to the remote V3 database connection string before running this script."
}

if ($TargetDbUrl -match "127\.0\.0\.1|localhost|54322|54321") {
  throw "TargetDbUrl looks local. This is the REMOTE import script. Not today, localhost goblin."
}
...
Write-Step "Using remote V3 target DB URL"
Write-Host $TargetDbUrl
...
psql $TargetDbUrl -c "select current_database(), current_user;"

Write-Step "Checking remote V3 counts before import"
psql $TargetDbUrl -c "
select
...
Write-Step "Importing old auth data into remote V3"
psql $TargetDbUrl -f $AuthDump
...
psql $TargetDbUrl -c "\copy public.migration_old_profiles from '$($ProfilesCsv.Replace('\','/'))' with csv header"
...
Write-Host ""
Write-Host "Remote auth/profile migration complete." -ForegroundColor Green
