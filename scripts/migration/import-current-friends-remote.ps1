...
# TriggerFeed V3 Remote Friends Migration
#
# Imports current legacy TriggerFeed friend relationships into REMOTE V3 Supabase.
#
# Requirements:
...
if ($TargetDbUrl -match "127\.0\.0\.1|localhost|54322|54321") {
  throw "TargetDbUrl looks local. This is the REMOTE friends import script. Not today, localhost goblin."
}
...
Invoke-Native "Validate staged friends" "psql" @(
  ...
  select count(*) as friends_without_target_user_profile
  from public.migration_old_friends f
  left join public.profiles p on p.id = f.user_id
  where p.id is null;

  select count(*) as friends_without_target_friend_profile
  from public.migration_old_friends f
