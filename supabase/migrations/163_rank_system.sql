...
create unique index if not exists user_rank_thresholds_sort_order_idx on public.user_rank_thresholds(sort_order);
insert into public.user_rank_thresholds (key, label, min_posts, sort_order, is_active)
values 
  ('FNG', 'FNG', 0, 10, true),
  ('range_regular', 'Range Regular', 10, 20, true),
  ('trailhand', 'Trailhand', 25, 30, true),
  ('camp_builder', 'Camp Builder', 50, 40, true),
  ('brass_stacker', 'Brass Stacker', 100, 50, true),
  ('gear_hound', 'Gear Hound', 200, 60, true),
  ('readiness_regular', 'Readiness Regular', 350, 70, true),
  ('signal_fire', 'Signal Fire', 500, 80, true),
  ('trail_boss', 'Trail Boss', 750, 90, true),
  ('brass_baron', 'Brass Baron', 1000, 100, true) on conflict (key) do
update
set label = excluded.label,
  min_posts = excluded.min_posts,
  sort_order = excluded.sort_order,
  is_active = excluded.is_active;
revoke all on public.user_rank_thresholds
from anon;
revoke all on public.user_rank_thresholds
from authenticated;
grant select on public.user_rank_thresholds to anon,
  authenticated;
alter table public.profiles
add column if not exists last_seen_rank_key text references public.user_rank_thresholds(key) on delete
set null,
  add column if not exists last_seen_rank_at timestamptz;
grant update (last_seen_rank_key, last_seen_rank_at) on public.profiles to authenticated;
create or replace function public.get_user_rank(p_user_id uuid) returns table (
    user_id uuid,
    post_count integer,
    rank_key text,
    rank_label text,
...
set search_path = public as $$ with target_profile as (
    select p.id
    ...
    from target_profile tp
      left join public.posts posts on posts.user_id = tp.id
...
from current_rank cr
  left join next_rank nr on nr.user_id = cr.user_id;
$$;
revoke all on function public.get_user_rank(uuid)
from public;
